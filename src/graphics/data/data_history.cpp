...
data_history::data_history(int length_, int width_) {
    if (length_ < 1) {
...
void data_history::update_data(data_channel new_data) {
...
data_channel data_history::get_current_data() {
    verify_data();
    return history[0];
}
std::vector<data_channel> data_history::get_data_history() {
    verify_data();
    return history;
}
